...
# ─── OAuth2 Google & GitHub (AUTH-02, AUTH-03) ────────────────────────────────

@router.get("/oauth/google")
async def oauth_google_login(request: Request):
    """
    AUTH-02: Initiate Google OAuth2 PKCE flow.
    ...
    try:
        ...
        # Store code verifier in Redis for callback verification
        r = await get_redis()
        if r:
            await r.setex(f"oauth:google:{state}", 600, code_verifier)  # 10 min expiry
...
async def oauth_github_login(request: Request):
    """
    AUTH-03: Initiate GitHub OAuth2 PKCE flow.
    Returns authorization URL with state and code challenge.
    """
    try:
        # Generate PKCE parameters
        ...
        r = await get_redis()
        if r:
            await r.setex(f"oauth:github:{state}", 600, code_verifier)  # 10 min expiry
        
        # Generate authorization URL
        auth_url = oauth_service.get_github_auth_url(state, code_challenge)
...
async def oauth_callback(
    ...
    """
    AUTH-02 & AUTH-03: OAuth callback handler for both Google and GitHub.
    ...
    try:
        ...
        code_verifier = await r.get(f"oauth:{provider}:{state}")
        if not code_verifier:
            raise HTTPException(status_code=400, detail="Invalid or expired state")
        
        # Clean up state
        await r.delete(f"oauth:{provider}:{state}")
        
        # Exchange code for token and user info
        if provider == "google":
            token_data = await oauth_service.exchange_google_code(code, code_verifier)
        elif provider == "github":
            token_data = await oauth_service.exchange_github_code(code, code_verifier)
        else:
        ...
        if not user:
            ...
            password_hash = auth_manager.hash_password(secrets.token_hex(32))
            display_name = user_info.get("name", f"{provider.capitalize()} User")
            
            user = await db.create_user(email, password_hash, display_name)
            await db.create_user_plan(str(user["id"]))
            await db.create_usage_quota(str(user["id"]))
            
            # Update user with provider ID
            if provider == "google":
                await db.update_user_google_id(str(user["id"]), provider_id)
            elif provider == "github":
                await db.update_user_github_id(str(user["id"]), provider_id)
        
        # Mark email as verified (OAuth providers verify email)
        if not user.get("email_verified", False):
            await db.verify_user_email(str(user["id"]))
        ...
        frontend_url = os.getenv("FRONTEND_URL", "http://localhost:3000")
        response = RedirectResponse(url=f"{frontend_url}/login?access_token={access_token}")
        response.set_cookie(
            key="refresh_token",
            value=refresh_token,
            httponly=True,
